...
import { Loader2, Search, AlertCircle, FileText, Image } from "lucide-react"
...
export default function Home() {
    ...
    const simulateProgress = () => {
      ...
      const interval = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          // Generate random similarity scores for demo
          const textScore = Math.floor(Math.random() * 100)
          const imageScore = Math.floor(Math.random() * 100)
          const overallScore = Math.floor((textScore + imageScore) / 2)
  ...
  const viewTextSimilarity = () => {
    router.push(`/text-similarity?source=${encodeURIComponent(sourceUrl)}&target=${encodeURIComponent(targetUrl)}`)
  }

  const viewImageSimilarity = () => {
    router.push(`/image-similarity?source=${encodeURIComponent(sourceUrl)}&target=${encodeURIComponent(targetUrl)}`)
  }
  ...
  return (
    <main className="container mx-auto py-8 px-4">
      ...
      {showResults && (
        <Card>
          <CardHeader>
            <CardTitle className="text-center">Similarity Analysis Results</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <SimilarityCard
                  ...
                  score={results.textSimilarity}
                  description="Based on product descriptions, features, and specifications"
                  onViewDetails={viewTextSimilarity}
                  icon={<FileText className="h-3.5 w-3.5 mr-1" />}
                />
                <SimilarityCard
                  title="Image Similarity"
                  score={results.imageSimilarity}
                  description="Based on product images, colors, and visual elements"
                  onViewDetails={viewImageSimilarity}
                  icon={<Image className="h-3.5 w-3.5 mr-1" />}
                />
                <SimilarityCard
                  title="Overall Similarity"
                  score={results.overallSimilarity}
              ...
              <div className="p-4 border rounded-md bg-muted/50">
                ...
                <p className="text-sm text-muted-foreground">
                  {results.overallSimilarity > 75
                    ? "These products show a high degree of similarity. There may be concerns about potential copyright infringement or product imitation."
                    : results.overallSimilarity > 50
                      ? "These products show moderate similarity. Some elements appear to be similar, but there are also notable differences."
                      : "These products show low similarity. They appear to be distinct products with different characteristics."}
                </p>
              </div>
            </div>
          </CardContent>
